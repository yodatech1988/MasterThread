...
| R4 | Finish `generate_agents_md.py`: carry Role as a frontmatter field (`role: A`, `R` or `D`) so it can be generated rather than hand-kept, generate all global tables, and add a `--check` mode that fails when the roster and `claude-agents/` disagree. Run `--check` in MasterThread CI. | F3, F4, F5 | M |
