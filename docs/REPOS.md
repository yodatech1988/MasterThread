...
| [aegis-mods](https://github.com/yodatech1988/aegis-mods) | Every AEGIS Workshop module: Metrics, TeddyBear, Economy, Skins merged. Plan for the clean-room replacement of the removed unsafe mod family (skills/perks, medicine, start screen) is merged ([#17](https://github.com/yodatech1988/aegis-mods/pull/17)) — no code yet | `docs/PLAN.md` (Sessions 1–11 `AEGIS_Core`, `AEGIS_Skills`, `AEGIS_Medicine`, `AEGIS_StartScreen`; 12–15 move PvPGuard, Vehicles, Aircraft, Quest 1033) | Round A in parallel: research lanes #9–#14 (issues, no PRs — none started as of 2026-09-14). Session 1: build `AEGIS_Core` | Faction-name mapping sign-off (site-chernarus `nasdara-factions-and-quests.md`, needed before Session 10, not before); confirm the removed mod family's Workshop items are unsubscribed on Steam (local files deleted 2026-09-14, subscription status not confirmed); Workshop publisher account when a module is ready |
...
| [aegis-pricing](https://github.com/yodatech1988/aegis-pricing) | `AEGIS_Pricing`: Expansion Market buy/sell prices scale by Hardline reputation | `docs/PLAN.md` ([#1](https://github.com/yodatech1988/aegis-pricing/pull/1), 7 sessions; Sessions 1–2 built and boot-tested on stacked PRs [#3](https://github.com/yodatech1988/aegis-pricing/pull/3)/[#4](https://github.com/yodatech1988/aegis-pricing/pull/4), none merged yet) | Merge #1, #3, #4 in order. Session 3: in-game verification — needs you at a trader with `@AEGIS_Pricing` in your own client mod list | Workshop publisher account before go-live (module must be client-loaded); site-chernarus market sign-off (#6) for the real curve numbers |
...
| [claude-agents](https://github.com/yodatech1988/claude-agents) | Community, Patreon, chat and economy agents; Discord ticket system | `docs/PLAN.md` grew to 9 sessions since #6. [#16](https://github.com/yodatech1988/claude-agents/pull/16) `bug-report-agent` (owner in-game `!bug` -> Claude-drafted `docs/PLAN.md` session -> PR) is **merged** (`6d38ed5`, 2026-09-14) — but only its first commit landed; 2 follow-up commits (`RconKey.ps1` tooling etc.) were pushed to the same branch after the merge and got stranded, so a separate session is landing that tooling properly in its own PR | Per that repo's own `docs/PLAN.md` status table: Session 7 (needs #12, already merged) is next, then 8, then 9 — its own PLAN.md status table looked stale too (marks Session 1/#15 "in review" though it's merged) and could use a refresh. `bug-report-agent` is deliberately **not running and holds no Anthropic key** — intentional, not stalled — pending `gh-federation`'s two-hop federated design (its Session 5) landing | None currently blocking; review/merge the RconKey tooling PR once it lands |
| [gh-federation](https://github.com/yodatech1988/gh-federation) | Federated (OIDC-based) GitHub write access for bots — a repo's own scheduled Actions workflow (OIDC-authenticated, exact repo+branch match) files its own issues/PRs for tasks another bot queued, so no bot ever holds a static cross-repo PAT/key. Created 2026-09-13 (Jeremy asked for it directly, no `aegis` prefix); `bug-report-agent` (claude-agents#16, merged) is waiting on this repo's two-hop design before it runs | `docs/PLAN.md` ([#1](https://github.com/yodatech1988/gh-federation/pull/1), 6 sessions). Session 0 (the plan) is **merged** (`0f2616e`). Next: a plan-amendment PR fixing 5 design gaps found in review — allow-list instead of live default-branch lookup, stricter OIDC claim checks, an Actions-minutes budget default, a CI-trigger workaround for `GITHUB_TOKEN`-opened PRs, and a two-hop WIF-based triage redesign for Sessions 4–5 | Once the plan-amendment merges: Session 1, OIDC verification code (`src/verifyOidc.js`) | Review/merge the plan-amendment PR, then #1's Session 1 |
...
| [claude-session-archive](https://github.com/yodatech1988/claude-session-archive) | Archives every Claude Code session transcript under `~/.claude/projects/` to this repo: redacted, full-text searchable (SQLite+FTS5), AI-summarized | `docs/PLAN.md` (6 sessions; [#1](https://github.com/yodatech1988/claude-session-archive/pull/1) plan, [#2](https://github.com/yodatech1988/claude-session-archive/pull/2)+[#3](https://github.com/yodatech1988/claude-session-archive/pull/3)+[#4](https://github.com/yodatech1988/claude-session-archive/pull/4) Session 1, [#5](https://github.com/yodatech1988/claude-session-archive/pull/5)+[#6](https://github.com/yodatech1988/claude-session-archive/pull/6)+[#7](https://github.com/yodatech1988/claude-session-archive/pull/7) Session 2, [#8](https://github.com/yodatech1988/claude-session-archive/pull/8) Session 3, [#9](https://github.com/yodatech1988/claude-session-archive/pull/9) Session 4 — all merged) | Session 5: `src/run.js` ties the pipeline together end to end (discover → backup → parse → redact → render → summarize → gitleaks gate → index → auto-merging PR). Opus 5 — a mistake here risks a real secret leak | None currently blocking. Standing open decision: raw backups stay unencrypted plaintext in `%LOCALAPPDATA%\ClaudeSessionArchive\raw\` (same exposure as `~/.claude` itself); revisit if the threat model changes |
...
| [claude-session-archive](https://github.com/yodatech1988/claude-session-archive) | Private repo (created 2026-09-13 from repo-template). Batch that reads every Claude Code session transcript from `~/.claude/projects/`, redacts secrets, and archives a readable transcript + AI summary here, with a SQLite full-text index rebuilt from the committed tree | `docs/PLAN.md` ([#1](https://github.com/yodatech1988/claude-session-archive/pull/1), 6 sessions; Session 0, [Session 1](https://github.com/yodatech1988/claude-session-archive/pull/2), [Session 1 follow-ups](https://github.com/yodatech1988/claude-session-archive/pull/3), and [Session 2](https://github.com/yodatech1988/claude-session-archive/pull/5) merged — `cleanupPeriodDays` raised to 3650, daily+logon scheduled task running from the stable main checkout, CI actually runs `npm test`, parse/redact/render built with 47 passing `node:test` cases covering every planted-secret rule) | Session 3: `migrations/0001_init.sql` + `src/index.js` + `src/search.js` (SQLite FTS5 index, porting the migration-runner pattern from `aegis-services/db/ops`) | A separate in-progress worktree (`agent/claude-session-archive/session2-redaction-gaps`, no PR yet as of 2026-09-14) may land a redaction fix before Session 3 starts — check open PRs first |
