...
| 5 | Push and PR the **map-parity canon change**, branch `docs/map-equality-and-seasons` in both `core` and `site-chernarus` (committed locally, not yet pushed as of 2026-09-12) | core, site-chernarus | See "Standing decision" below. Both branches are Session-0-adjacent — either lands as its own PR before each repo's Session 0, or gets folded into it. |
...
Recorded in `core` `docs/canon/aegis-cross-map-progression.md` (canonical) and mirrored in
`site-chernarus` `docs/aegis-cross-map-progression.md` (a stale full duplicate that should eventually
become a pointer stub — the earlier canon migration that was supposed to leave stubs in
site-chernarus never fully took effect; `aegis-network-canon-bible.md` there had already drifted from
core's copy before this session touched anything). `site-chernarus`'s canon bible, technical
requirements, both site dossiers, the go-live plan, the questlines doc, and the site-dossier template
were all updated to match. Work is committed locally on branch `docs/map-equality-and-seasons` in
worktrees `_wt-core-map-equality` and `_wt-chernarus-map-equality`; **not pushed yet** — push and open
PRs, then close out item 5 above.
...
| [core](https://github.com/yodatech1988/core) | AEGIS shared tooling, canon, reusable CI | none | Push branch `docs/map-equality-and-seasons` (`_wt-core-map-equality`) and PR the map-parity canon rewrite first; then [Session 0 (#44)](https://github.com/yodatech1988/core/issues/44) | Merge/close PRs #35 #39 #41 #42 #43 |
...
| [site-chernarus](https://github.com/yodatech1988/site-chernarus) | AEGIS Chernarus server config | none | Push branch `docs/map-equality-and-seasons` (`_wt-chernarus-map-equality`) and PR the map-parity doc updates first; then [Session 0 (#44)](https://github.com/yodatech1988/site-chernarus/issues/44), whose first job is the 15-PR backlog | Production instance decision (#5); market sign-off (#6) |
