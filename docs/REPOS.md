...
| 9 | ~~Merge core PR #71~~ (Session 4, `docs/ops/VENDORED-VALIDATOR.md`) — merged 2026-09-14 | core | Session 5 (`v0.1.0` tag) can now start. |
...
| 11 | ~~Approve `ECONOMY_CATALOG_SYNC_ENABLED=true`~~ — approved and set 2026-09-14 12:31Z; manual run inserted 24, deactivated 263 orphans, 0 referenced | services #58, #74 | Still open: first green *scheduled* run of each economy workflow. |
...
| 14 | ~~Decide the `aegis-mods` draft-PR triage table on #21~~ — owner chose "apply triage" 2026-09-14: research docs merge as docs, #24 folds into #27, #29's Mining code held on a branch for a build session | aegis-mods #21 | Being applied by a parallel lane. |
| 15 | Set PayPal as the payment method on **Shockbyte** (open an invoice → "PayPal - Subscribe" for auto-renewal) and **OVH** (Account → Payment methods → Add PayPal), from your own live PayPal login | Shockbyte billing, OVH Manager | 2026-09-15 decision: PayPal funds hosting renewal plus car/creature/gun marketplace model purchases (see `aegis-marketplace-research`). Both vendors natively support PayPal — no card/PIN storage needed anywhere. Not started as of this refresh. |
| 16 | ~~Merge services#94~~ (adds `aegis-marketplace-research` to the VPS-CI runner allowlist) — merged 2026-09-16. Still open: run `GitHub\AEGIS-VPS-CI-All.cmd`, then restore that repo's branch protection per services#95 (open issue, not a PR) | services PR #94 (merged), services issue #95 (open) | New repo's CI has queued with no runner since creation; its branch protection required check was temporarily removed to let its Session 0 PR merge. services#95 has the full restore checklist. |
...
| [aegis-pricing](https://github.com/yodatech1988/aegis-pricing) | `AEGIS_Pricing`: Expansion Market buy/sell prices scale by Hardline reputation | `docs/PLAN.md` ([#1](https://github.com/yodatech1988/aegis-pricing/pull/1), 7 sessions). Sessions 1 (#3) and 2 (#4) merged — **module is live on production** (site-chernarus #67 re-enabled it there, production instance recorded, RCon port fixed) | Session 3 (in-game verification, tags v0.1.0/v0.2.0) is queued for the owner's in-game window later 2026-09-14 (not started) | In-game verification window (Do-first #13); Workshop publisher account before go-live; site-chernarus market sign-off (#6) for the real curve numbers |
| [aegis-marketplace-research](https://github.com/yodatech1988/aegis-marketplace-research) | Research and license-track third-party marketplace sources (vehicles, creatures, weapons) for trader content — decisions/metadata only, never the asset binaries | `docs/PLAN.md` ([#1](https://github.com/yodatech1988/aegis-marketplace-research/pull/1), 4 sessions: vehicle/creature/weapon catalogs, then a purchase-decision process doc). **Session 0 merged 2026-09-15** — its CI check never ran (no VPS runner registered for this repo yet, see Do-first #16), so the required check was temporarily dropped from branch protection to let it merge | Session 1 (vehicle catalog, transcribing the existing `dayz-vehicle-sources` local-folder research) can start any time — not blocked on the runner fix, which only affects PR *checks*, not the work itself | Merge services#94, run `AEGIS-VPS-CI-All.cmd`, restore branch protection (Do-first #16). Purchases themselves stay owner-only per standard rule 7 — this repo only researches and recommends |
| [site-chernarus](https://github.com/yodatech1988/site-chernarus) | AEGIS Chernarus server config | `docs/PLAN.md` ([#53](https://github.com/yodatech1988/site-chernarus/pull/53), 5 sessions). Session 1 (#61) merged; Session 2 superseded (answered directly by the 2026-09-14 owner-decisions comment on #41); Session 3 weapon nominals (#69) merged; **Session 4 economy defects + golive-doc reconciliation (#80) merged 2026-09-14** | **Session 5 rarity↔price (#81) merged 2026-09-14** (ρ 0.544→0.718, not yet pushed live); Sessions 6+ go-live plan being written. 18 off-plan PRs also merged 2026-09-14 (#62–#78, listed in the repo's own PLAN.md) — most notably **#78 re-vendors `restart_orchestrator.py` from core**, closing the swallowed-TransportError live-restart risk, and #77 adds MiniMap Relocated (merged, not yet deployed) | Market sign-off (#6); walk the MiniMap Workshop-folder confirmation + restart + `.bikey` check with the re-vendored orchestrator, `LOOT_DEPLOY_ENABLED` stays off (owner declined 2026-09-14); flip `METRICS_DEPLOY_ENABLED` (Do-first #8) |
...
| [claude-session-archive](https://github.com/yodatech1988/claude-session-archive) | Archives every Claude Code session transcript under `~/.claude/projects/` to this repo: redacted, full-text searchable (SQLite+FTS5), AI-summarized | `docs/PLAN.md` (6 sessions; Sessions 1–4 all merged). Session 5 merged as #11 (2026-09-14). **Session 6 (backfill) started 2026-09-18: PR #14 merged, 11 of 296 discovered sessions archived (0 gitleaks findings), the rest deferred by design (`--limit` batching, per Session 5's own implementation notes) — not yet complete** | Session 6, continued: run further `node src/run.js --limit N` batches until every discovered session has a `manifest.json` entry (285 remained after PR #14), then spot-check rendered output and this row again | None currently blocking. Standing open decision: raw backups stay unencrypted plaintext in `%LOCALAPPDATA%\ClaudeSessionArchive\raw\`; revisit if the threat model changes |
...
| [ops-infra](https://github.com/yodatech1988/ops-infra) | Hardening for the vault and edge hosts: rootless Podman per zone, SOPS/age secrets, auditd/AIDE/CrowdSec, LUKS containers, AppArmor. Every merge needs owner approval. | `docs/PLAN.md`. Phase 1 sessions 1-4 (#2-#5), session 3 audit (#4), session 4 secrets (#5), session 7 encryption (#8) and session 11 AppArmor (#7) all merged. **#9 (maintenance-window gate + vps-drift-checker) and #10 (session 8: platform Postgres + hash-chained audit log) open** | Merge #9 and #10 | Review and merge #9, #10 |
| [ops-platform](https://github.com/yodatech1988/ops-platform) | The autonomous ops cycle's runtime: PM, router, gateway, cost estimator, ledger/accountant, batch lanes | `docs/PLAN.md`. Sessions P2-P7 (router #3, ledger/accountant #5, batch #6, project-manager #8) all merged; estimator (#4) and Discord bridge (#7) closed/superseded. **Code is complete and tested but not yet deployed to the vault** — needs the vault's own SPIRE/federation runtime, a separate not-yet-started work item. #10 (T4 subagent: budget-envelope-reporter) open | Vault deployment (SPIRE/federation) is the real next step, not yet started as a numbered session | Review and merge #10; unblock the vault-deployment work |
| [ops-business](https://github.com/yodatech1988/ops-business) | QuickBooks and property-management modules for the personal/financial enclave | `docs/PLAN.md`. Phase 4 plan (#1) merged 2026-09-15 — plan only, no build sessions started yet | Session 1 not yet started | Nothing blocking; early-stage, plan-only |
| [ops-household](https://github.com/yodatech1988/ops-household) | Family profiles, calendar sync, Discord reminders, right-to-delete, for the personal/financial enclave | `docs/PLAN.md`. Phase 5 plan (#1) merged 2026-09-15 — plan only, no build sessions started yet | Session 1 not yet started | Nothing blocking; early-stage, plan-only |
...
| Path | Area | Note |
|---|---|---|
| `_security-public/policies/` | Org-wide policy of record: C0-C3 data classification, the two-enclave (game-network / personal-financial) model, and the 7 governing rules every repo (core, site-chernarus, services, claude-agents, aegis-mods, MasterThread) derives from | Written 2026-09-12, extended 2026-09-14. No `.git` directory — plain files, no remote, no PRs possible until it becomes a real repo. Program status per owner decision: **paused**, planned but not deployed; resume documented for 2026-09-19 (scheduled task). Cross-referenced against `ops-policies`' D0-D3 scheme in `ops-policies` PR #13, which found one live conflict (Q32) needing an owner call before either scheme changes. A drafted `security/enclaves.md` amendment (adds the platform layer, currently missing from the two-enclave model) is held by the PM, ready to drop in once this repo exists. |
| `_security-aegis/` | Game-network enclave's private security/ops content (mirrors `_security-public`'s structure for game-side specifics) | `git init`'d locally, **zero commits**, no remote. Same paused-program status as `_security-public`. |
| `_security-personal/` | Personal/financial enclave's private security/ops content | `git init`'d locally, **zero commits**, no remote. Same paused-program status as `_security-public`. |
