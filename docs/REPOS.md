...
| 16 | Merge **services#94** (adds `aegis-marketplace-research` to the VPS-CI runner allowlist — deliberately not auto-merged, see its body), then run `GitHub\AEGIS-VPS-CI-All.cmd`, then restore that repo's branch protection | services #94, services #95 | New repo's CI has queued with no runner since creation; its branch protection required check was temporarily removed to let its Session 0 PR merge. services#95 has the full restore checklist. |
