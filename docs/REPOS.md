...
| 15 | Review and merge the 3 **vault/black-market portal** design PRs: site-chernarus #92 (the design doc itself), aegis-poi #10 (Sessions 7-8), services #93 (Sessions P1-P2) | site-chernarus, aegis-poi, services | Design-only, docs-only across all three — nothing downstream can be scheduled or built until these land. See the standing-initiative entry below for the full shape. |
...
## Standing initiative: vault/black-market portal network

Added 2026-09-14. Cross-repo, not owned by any single repo's ledger row. Design doc:
`site-chernarus` `docs/aegis-vault-blackmarket-portal-network.md`
([site-chernarus#92](https://github.com/yodatech1988/site-chernarus/pull/92)). Goal: route players
from Chernarus to two new hidden, unlisted Shockbyte instances (Black Market, Dungeons/Vaults) via
an in-game airlock/punch-card door, so high-AI vault content stops sharing simulation budget with
the live population map.

**Decided:** two separate new Shockbyte instances, not one combined server; punch card required
once per group (not per person), consumed at the door; single universal exit per hidden instance;
fiction ties the vaults to Castellan's pre-collapse labs beneath the town, reachable because the
Collectors control the supply tunnels (consistent with their existing corridor-operator role,
`aegis-chernarus-questlines.md` §3.4 — not a new faction).

**Not yet decided** (design doc §11): the AI-difficulty formula; visibility mechanism (Shockbyte
panel toggle vs. firewall, needed for two instances); which service owns the shared-backend routes
long-term (default `community-api`, unconfirmed); whether Black Market and Dungeons/Vaults need a
direct portal between each other (default: no); punch-card sourcing/economy placement.

**Repos and sequencing** (design doc §9, mirrored per-repo):
- `site-chernarus` — holds the design doc itself (#92). No build session of its own; this repo's
  job is documentation + eventually installing finished modules.
- `services` — new vault-portal track, Sessions P1-P2 (#93). **Blocks everything below** — no
  downstream session can be built against a real API without P1.
- `aegis-poi` — new Sessions 7-8 (#10), after the existing Session 5 (Vault base, not started) and
  Session 6 (stabilise 1.0.0, not started).
- **Two new site repos, not yet created** — one per hidden instance (Black Market, Dungeons/Vaults),
  named TBD. Each needs its own Session 0 scaffolding (mirrors `aegis-poi`/`site-chernarus`'s own
  pattern) once hosting is provisioned. Add rows to the Active table below when these exist — don't
  create them speculatively; that's an owner-driven step tied to actually provisioning the two
  Shockbyte slots.
- `aegis-mods` — no new session; `AEGIS_Core`'s `AegisCorePlayerStore<T>` pattern gets reused, not
  extended, by `aegis-poi`'s Session 7.

**Owner steps that block live testing (not code-writing):** provisioning both new Shockbyte slots;
confirming visibility (panel toggle vs. firewall) for each.
