...
Status: **DRAFT for owner review, 2026-09-21; updated 2026-09-22 to reflect merged work.** Plan
only in its reasoning and open items, but wave 1-3 of §6's build plan has since merged: F2 (#174),
F10 (#172), F3 (via #174/#176), F4 (#190), F12 (#197, wrappers merged — resume/fork/fallback-model
threading is tracked as a follow-up, not "not built"), F9 draft (#195), F8 (#202) and F14 (#203) are
all merged to `origin/main`; see §6's status table and §7 for exactly what is and is not verified.
Nothing else below is dispatched, no other standard is edited, no Decision Queue card is filed by
this document itself. **This is a planning artifact: the session that wrote it does not build it.**
...
classifies as read-only, and nothing documents that classifier's exact list. The fix round
(2026-09-22, same CLI, same flags) probed the egress and outside-the-working-directory cases:

- With `--allowedTools "Bash(echo:*)"`: `curl -s … https://example.com`, `gh api /zen` and
  `git ls-remote https://github.com/…` were each **denied** and recorded in `permission_denials`.
  So were `cat` and `ls` on an absolute path outside the working directory. `whoami` ran again.
- With `--allowedTools "Bash(curl:*)"`, which explicitly allows curl, `curl` was still **denied**:
  `readonly.settings.json`'s `Bash(curl *)` deny rule overrides the allow. The deny list is a
  working backstop for the commands it names.

That is evidence for the commands probed, not a proof for every command. The claim this plan makes is
...
list names (denied, proving the deny list is still the backstop). The `<exact list>` is a placeholder,
...
not a boundary. With these flags, an unattended Fable call has only `Read`, `Grep` and `Glob`, and `--restricted`
...
unattended resume has no tool beyond `Read`, `Grep` and `Glob`, and that each of the three is denied
on an absolute path outside the working directory.
...
| F10 | #172 | **merged to main** 2026-09-22T02:18:45Z (commit `092d278`) | Route C (`standards/sessions/*`); one-file diff. |
| F3 | #176 | **merged to main** 2026-09-22T02:18:24Z into #174's branch, landed on `main` via #174's squash (commit `10786c2`) | #176 squash-merged into `agent/MasterThread/fable-f2` as `8dab62c` at 02:18:24Z; that became #174's head and landed on `main` in the same `10786c2` squash. No separate F3-to-main PR is needed. |
...
| F4 | #190 | **merged to main** 2026-09-22T02:46:38Z (commit `0a5ff4e`) | Schemas only — see F4's Group A row for what is out of scope (per-agent verb pinning was #208's phase 1, not F4; #208 is now MERGED, closing issue #204). Opened wave 2, merged via Merge Authority. |
| — | #159 | **merged to main** 2026-09-22T03:17:07Z (commit `2575bf1`) | Cost-monitor Phase 1. This clears §7 condition 4 and F8's stated dependency; F8 is no longer "Blocked on #159" (see F8's Group B row). |
| F12 | #197 | **merged to main** 2026-09-22T14:59:24Z (commit `ec5e0c7`) | Seat-side wrappers (`Ask-Fable`, `Invoke-Lane`, `Invoke-Subagent`) with `maxTurns`/budget enforcement. **Merged with resume/fork/fallback-model threading outstanding**, not fully DONE: `Ask-Fable` does not yet thread `--session-id`/`--resume`/`--fallback-model`/`--fork-session` to the real `claude` call, so today's seat is single-turn per invocation. Two duplicate follow-up issues track this (#198, #205); a third follow-up, #204 (fine-grained `--allowedTools` for `pr-state-sweep`'s `gh` subcommands), is **CLOSED** by its fix PR **#208**, which is **MERGED to main** 2026-09-22T17:26:04Z (squash `c1200827b4f3cfda49c20067a02ce357a53aea37`, route B). **New finding from #208's live acceptance case, not a #208 defect:** issue #212 records that a live `pr-state-sweep` run returns markdown prose instead of schema-valid JSON under `--json-schema` (exit 8) — pre-existing agent-output-conformance gap, not something #208's files touch. |
| F14 | #203 | **merged to main** 2026-09-22T14:59:52Z (commit `928b77d`) | Publishes the F-task tracker artifact. |
| F9 | #195 | **merged to main** 2026-09-22T16:38:18Z (commit `9c7e791`) | Draft of the continuity contract landed at `docs/drafts/fable_seat_draft.md`, not yet at its intended location `standards/sessions/fable_seat.md`. Relocation PR **#213** ("standards: fable_seat.md (F9) relocated from drafts, TODO(F12) resolved", route C) moves it to that path and resolves the TODO(F12) note — **#213 is OPEN, awaiting owner merge** as of this writing. Read the draft at its current path until #213 merges. |
| F8 | #202 | **merged to main** 2026-09-22T16:29:20Z (commit `56b9a63`) | Resolves the cache-write TTL pricing ASSUMED range (§1b) and adds the headless drop-folder ledger. Its dependency #159 merged first (above), so this closes cleanly, not as a deferral. |

F2, F3, F10, F4, #159, F12, F14, F9 and F8 are all on `main` as of the timestamps above (F2 and F3
via the same `10786c2` commit). The task rows below still describe the work as planned in most of
their prose; where a row has since merged, its own line notes that and what, if anything, is still
open (F12's resume threading, F9's file location).
...
| F4 | One `--json-schema` per L1 reporter under `tools/headless/schemas/` | agent / M / Sonnet / low | F3 | **Merged as PR #190** (`0a5ff4e`, 2026-09-22T02:46:38Z). **Shipped schemas only** — per-agent verb pinning (the `<exact list>` §5 describes for row-7 `--allowedTools`) is not part of this PR; it was pinned by **#208** (phase 1, `pr-state-sweep` only), **MERGED to main** 2026-09-22T17:26:04Z (`c1200827b4f3cfda49c20067a02ce357a53aea37`, route B), closing issue #204. Schema per agent, matching that agent's documented output section. Makes the L2 comparator's input machine-checkable instead of prose-parsed. |
| F14 | Publish the F-task tracker artifact (db capability, `items` + `meta/summary` per §2's tracker subsection) and backfill F1a/F1b as `done` | seat (interactive; not a headless agent) / XS / Sonnet / low | F1a, F1b | Same shape as the reference tracker at https://claude.ai/artifact/9ahVNbZjAT1tjrm8NCFXc5. The URL grants nothing by itself, because artifacts are private by default. This repo is public, so neither the reference tracker nor the F14 page may be link-shared publicly or shared with write access beyond the owner and the seat. The seat publishes it directly — `Artifact`/`ArtifactData` are seat-held tools, not something a `claude -p` agent can be handed (§2 mobile rule 2). Must exist before the first F-task after F1a/F1b is dispatched. Written by the Sonnet seat per owner instruction 2026-09-22, not by a headless agent. The brief must say which tracker rules the page enforces and which stay convention: the page's write code enforces the field set and a length cap on `note` (one line); "status only, never code, logs, secrets or a diff" is convention, because no schema can tell a short secret from a short status. As a cheap tool-level backstop, the page's write code must also reject a `note` containing a known token prefix or key marker (`ghp_`, `gho_`, `github_pat_`, `sk-`, `xox`, `AKIA`, `-----BEGIN`) or a long high-entropy run. That catches the common cases, not every secret. **Published 2026-09-22** at https://claude.ai/artifact/EmYPvwRz4PGbJCFkRmQu7X (private; owner + seat only, per the sharing rule above).

**Tracker URL note (for the PM to resolve, not resolved here):** three distinct artifact URLs
appear across this document set and are not the same thing. `https://claude.ai/artifact/EmYPvwRz4PGbJCFkRmQu7X`
is this F14 tracker (published above). `https://claude.ai/artifact/9ahVNbZjAT1tjrm8NCFXc5` is
called the "launch tracker" / "reference tracker" elsewhere in this section and in
`PM_INBOX/fable-launch/ROSTER_BRIEFS.md` §(a). `https://claude.ai/artifact/2dR2TXoALjX9F8ZRjSov9x`
is called the "Execution Board" in `ROSTER_BRIEFS.md` §2.1's Inputs row. This document does not
pick one as canonical; the PM should confirm whether these are three separate live artifacts, two
names for one, or a stale reference that needs retiring. Seeded with all 16 F-task rows (`F1a`…`F14`) from this table's own then-current state, not only F1a/F1b, so the board matches what had already merged (F2 #174, F3 #176, F4 #190, F10 #172) rather than presenting it as not-yet-done; F8's PR #159 dependency, merged 2026-09-22T03:17Z while this task ran, was reflected too. Field-set and note-length-cap enforcement, plus the token/key-marker and high-entropy backstops, are implemented as a `validateItem` function in the page (`window.__validateF14Item`); since this page has no viewer-facing write affordance (only the seat writes, via `ArtifactData`), that function is not yet wired into a live write path — the seat applies the same checks by hand before each write today, and the function exists so a future write UI enforces the identical rule rather than a looser one. This gap is recorded here rather than silently assumed closed. |
...
| F8 | Wire `total_cost_usd`/`usage` from the drop folder into `tools/cost-monitor` as a headless ledger source | agent / M / Sonnet / medium | F3, PR #159 merged | **Merged as PR #202** (`56b9a63`, 2026-09-22T16:29:20Z). #159 merged first (`2575bf1`, 2026-09-22T03:17:07Z), so F8 is **no longer "Blocked on #159."** Resolves the cache-write TTL pricing per the §1b finding that the 1h multiplier measured at 2×, closing that file's ASSUMED range; also adds the headless drop-folder ledger. |

### Group C — the Fable seat (owner-gated)

| ID | Task | Who / size / model / effort | After | Brief |
|---|---|---|---|---|
| F9 | Draft `standards/sessions/fable_seat.md`: the continuity contract — memory is a claim not evidence, cache over a durable record, fork for speculation; plus what it may produce, never-builds and never-merges, `--fallback-model`, refusal handling, the resume contract, the retirement/cold-reopen trigger, and a position on `--autocompact` | agent / M / Sonnet / medium | F12 | **Draft merged as PR #195** (`9c7e791`, 2026-09-22T16:38:18Z) — landed at `docs/drafts/fable_seat_draft.md`, not yet at its intended `standards/sessions/fable_seat.md`. Relocation PR **#213** ("standards: fable_seat.md (F9) relocated from drafts, TODO(F12) resolved") is **OPEN, awaiting owner merge**, route C (the destination is `standards/sessions/*`); until it merges, the draft is not the ratified standard. **Route C** (`merge_authority.md`): `standards/sessions/*` is owner-merge. A session drafts it; the owner merges it. The drafter must not also be its reviewer. |
| F10 | Correct `orchestrator_role.md`'s "Effort for background workers can't be set per call" to distinguish the `Agent` tool (model only) from `claude -p` (`--effort`), and add the row-0 Fable line to the model/effort table | agent / S / Sonnet / low | F1a | **Route C**, owner-merge. Smallest possible edit; it is a factual correction plus one row, not a rewrite. **Merged as PR #172** by the owner's account, 2026-09-22T02:18:45Z (`092d278` on `origin/main`, route C) — one-file diff to `orchestrator_role.md`: corrected the sentence quoted in finding F1 above (§1a) and added the row-0 Fable line from §4's table. Dispatched while the §7 gate was closed (see §7). Row 1 stays Opus (D2); #172 does not touch it. |
...
4. PR #159 (cost-monitor) is merged, or F8 is explicitly deferred. — **Satisfied 2026-09-22 by
   the first branch, not the second.** PR #159 merged 2026-09-22T03:17:07Z (commit `2575bf1`,
   verified via `gh pr view 159`). The deferral question (card `fable-confirm-f8-deferral-2026-09-22`)
   was never answered by the owner and is superseded, not resolved: the owner instead reopened F8
   and it has since merged as PR #202 (§6). This condition is read here as met on the condition's
   own literal text ("#159 merged, or F8 deferred") — the merge, not an owner statement about
   deferral, is what closes it.
...
**The gate's own conditions, stated plainly, 2026-09-22:** conditions 1, 2, 4 and 5 are verified —
each above cites the merged PR or resolved card that verifies it. **Condition 3 (the `readonly`
classification in `roster_meta.json` confirmed on `origin/main`) is not verified by this document**
— it was not re-checked this pass, and this document does not assert it. This plan does not declare
the gate fully open: a merged PR body verifying condition 3 is what would close it, and none has
been found. Every F-task dispatched after this document's original writing has run despite that gap,
same as the three exceptions recorded below — this is a fact about what has happened, not an
assertion that the gate is open. Once condition 3 is verified, what else blocks a given F-task is
that task's own dependency column in §6, plus the relayed owner WAITs in §9 (F5 and F11 remain WAIT
...
## 9. Open decisions (items 1–6 were drafted as questions and not filed as cards; the follow-up PR filed ten cards, listed at the end of §9)
...
> | F5 WAIT | Card `fable-confirm-f5-wait-2026-09-22` |
> | F11 WAIT | Card `fable-confirm-f11-wait-2026-09-22` |
> | F8 deferral | Card `fable-confirm-f8-deferral-2026-09-22` (it also gates §7 condition 4) |
...
- **D3 (item 3, `--restricted` default).** **`--restricted` is ON BY DEFAULT for every agent whose
  ...
  if the roster lookup can't resolve. F2 in §6 stays open until #174 is merged on `origin/main`.
...
- **F5 and F11 — WAIT.** Both stay undispatched by owner instruction 2026-09-22 (relayed),
  ...
  (the Decision Queue card authorizing the Fable seat) do not proceed until the owner says go. Both
  WAITs are on their own confirmation cards (table above). A relayed WAIT is still honoured while its
  card is open: holding is the fail-closed reading.
...
- **F8 — deferred (relayed).** Cross-reference for entry-gate condition 4: that condition stays
  **pending** until the owner answers card `fable-confirm-f8-deferral-2026-09-22`. F8 itself is not
...
The five `fable-confirm-*` cards are in the table above. The other five:
...
- `fable-handoff-exec-authority-confirm-2026-09-22`: is the execution-authority ruling recorded in
  `docs/handoffs/2026-09-22-fable-plan-to-windows-rc.md` §3 the owner's?
- `fable-l1-pilot-pinned-ref-2026-09-22`: should `AEGIS-L1Pilot` run a pinned, reviewed ref instead of
  re-fetching from `origin/main` (§1c)?
...
- **Re-verified by the follow-up PR (2026-09-21), superseding parts of the bullet above**: live
  ...
  resume pair). The second review-fix round added four more Haiku probes (§5): network reads on the
  row-7 line, the deny list overriding an explicit curl allow, `cat`/`ls` outside the working
  directory, and `Read`/`Grep` outside the working directory under `--restricted`.
- **Deliberately absent**: any code, any edit to a standard, any dispatch, any date commitment, any
  ...
  filed ten (listed in §9): three for questions it raised, five to confirm relayed owner decisions,
  one on the handoff's relayed execution-authority ruling, and one on the L1 pilot's self-refresh.
  It resolved none.
...
### Tie to F8 (deferral relayed) — what can land without it

F8 (wiring `total_cost_usd`/`usage` into `tools/cost-monitor`) is blocked on PR #159, and its deferral
is relayed, pending card `fable-confirm-f8-deferral-2026-09-22` (§9). None of the
above needs F8:
