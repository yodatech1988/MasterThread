...
| `standards/sessions/headless_readiness_ladder.md` | **IN FORCE as of 2026-09-22** (PR #170 merged 2026-09-22T00:52Z; confirmed on `origin/main`); fleet still at **L0** until L1's own 7-day exit criterion is met — but see the next row |
| Windows scheduled task `AEGIS-L1Pilot` | **Found registered on the owner's PC on 2026-09-21, not recorded anywhere in this plan or in the ladder's build-state table.** Its description reads "AEGIS L1 pilot reporter (headless_readiness_ladder.md) -- read-only, self-refreshing from origin/main"; it runs `%APPDATA%\AEGIS\l1-pilot\Run-L1Pilot.ps1` (generated by `AEGIS-Register-L1Pilot.ps1`, start boundary 2026-09-19) and writes to `%APPDATA%\AEGIS\reports\`; its last run returned 0. The ladder on `origin/main` still says "L1 scheduled task … **Not built**". This plan records the observation only: it does not decide whether the pilot counts as L1, or how it relates to F5's WAIT (§9). That is an owner question (see the follow-up PR body). |
...
4. ~~PR #159 (cost-monitor) is merged, or F8 is explicitly deferred.~~ — **Satisfied 2026-09-22.**
   The owner explicitly deferred F8 (owner decision 2026-09-22). PR #159 remains open/draft
...
**Resolved 2026-09-22 — owner decisions:**
...
- **D6 (item 6, drop-folder location).** **Reports stay on this PC** (`%APPDATA%\AEGIS\reports\`).
  ...
  seam.
- **F5 and F11 — WAIT.** Both stay undispatched by owner instruction 2026-09-22, independent of
  their dependency columns clearing. F5 (register the scheduled run — the L1 climb itself) and F11
...
- **Re-verified by the follow-up PR (2026-09-21), superseding parts of the bullet above**: live
  ...
  `claude -p --model haiku --output-format json` run (§11).
...
number yet (see the follow-up PR's open questions).
...
Drafted rider for §5 (not owner-approved; see the PR's open questions):

> **The same argument applies to row-1/row-2 review, not only to sweeps.** A round that opens
> several PRs and reviews each with its own `live-reviewer`/`diff-reviewer` invocation pays the cold
> preamble once per PR, exactly as N independent reporter schedules did in the measurement above.
> Where a round's PRs are reviewable together (same repo, same session, no intervening merge that
> would change what a later PR's diff means), dispatch **one** reviewer invocation that processes
> the round's PR batch in sequence within a single warm session, rather than one cold spawn per PR.
> This does not change *who* reviews row-1 work (still Opus, per D2) or weaken independent review —
> it changes the invocation shape, the same way F5's design turns N scheduled tasks into one.
...
instrumentation, just reading a field that already exists. Drafted rider for F3/F4/F8's briefs (§6):
...
- The one-reviewer-per-round batching rule is a dispatch-pattern rule for the seat, not a metering
  feature.
