...
| F4 | #190 | **merged to main** 2026-09-22T02:46:38Z (commit `0a5ff4e`) | Schemas only — see F4's Group A row for what is out of scope (per-agent verb pinning is #208's phase 1, not F4). Opened wave 2, merged via Merge Authority. |
...
| F4 | One `--json-schema` per L1 reporter under `tools/headless/schemas/` | agent / M / Sonnet / low | F3 | **Merged as PR #190** (`0a5ff4e`, 2026-09-22T02:46:38Z). **Shipped schemas only** — per-agent verb pinning (the `<exact list>` §5 describes for row-7 `--allowedTools`) is not part of this PR; it is pinned by #208 (phase 1, `pr-state-sweep` only), which was still open as of this writing. Schema per agent, matching that agent's documented output section. Makes the L2 comparator's input machine-checkable instead of prose-parsed. |
