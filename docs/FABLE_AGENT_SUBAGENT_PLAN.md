...
| F12 | #197 | **merged to main** 2026-09-22T14:59:24Z (commit `ec5e0c7`) | Seat-side wrappers (`Ask-Fable`, `Invoke-Lane`, `Invoke-Subagent`) with `maxTurns`/budget enforcement. **Merged with resume/fork/fallback-model threading outstanding**, not fully DONE: `Ask-Fable` does not yet thread `--session-id`/`--resume`/`--fallback-model`/`--fork-session` to the real `claude` call, so today's seat is single-turn per invocation. This is tracked by #198 (open); the duplicate #205 is now CLOSED as a duplicate of #198. A third follow-up, #204 (fine-grained `--allowedTools` for `pr-state-sweep`'s `gh` subcommands), is **CLOSED** by its fix PR **#208**, which is **MERGED to main** 2026-09-22T17:26:04Z (squash `c1200827b4f3cfda49c20067a02ce357a53aea37`, route B). **New finding from #208's live acceptance case, not a #208 defect:** issue #212 records that a live `pr-state-sweep` run returns markdown prose instead of schema-valid JSON under `--json-schema` (exit 8) — pre-existing agent-output-conformance gap, not something #208's files touch. |
| F14 | #203 | **merged to main** 2026-09-22T14:59:52Z (commit `928b77d`) | Publishes the F-task tracker artifact. |
| F9 | #195, #213 | **merged to main** 2026-09-22T16:38:18Z (commit `9c7e791`); relocation **merged** 2026-09-22T17:53:49Z (commit `1fe6c04`) | Draft of the continuity contract originally landed at `docs/drafts/fable_seat_draft.md`. Relocation PR **#213** ("standards: fable_seat.md (F9) relocated from drafts, TODO(F12) resolved", route C) moved it to `standards/sessions/fable_seat.md` and resolved the TODO(F12) note — **#213 MERGED to main** 2026-09-22T17:53:49Z (commit `1fe6c04`). Read the contract at its ratified path, `standards/sessions/fable_seat.md`; the old drafts path no longer applies. |
...
| ID | Task | Who / size / model / effort | After | Brief |
|---|---|---|---|---|
| F9 | Draft `standards/sessions/fable_seat.md`: the continuity contract — memory is a claim not evidence, cache over a durable record, fork for speculation; plus what it may produce, never-builds and never-merges, `--fallback-model`, refusal handling, the resume contract, the retirement/cold-reopen trigger, and a position on `--autocompact` | agent / M / Sonnet / medium | F12 | **Draft merged as PR #195** (`9c7e791`, 2026-09-22T16:38:18Z) — originally landed at `docs/drafts/fable_seat_draft.md`. Relocation PR **#213** ("standards: fable_seat.md (F9) relocated from drafts, TODO(F12) resolved") **MERGED to main** 2026-09-22T17:53:49Z (commit `1fe6c04`), route C (the destination is `standards/sessions/*`); the ratified standard now lives at `standards/sessions/fable_seat.md`, and the old drafts path no longer applies. **Route C** (`merge_authority.md`): `standards/sessions/*` is owner-merge. A session drafts it; the owner merges it. The drafter must not also be its reviewer. |
